<?php
  ...
  if ($_SERVER ['REQUEST_METHOD'] === 'POST') {
      ...
      if (empty($felhasznalonev_err) && empty($jelszo_err) && empty($confirm_jelszo_err)) {
          $sql = 'INSERT INTO felhasznalok (felhasznalonev, jelszo, jog) VALUES (?,?, 3)';
          if ($stmt = $mysql_db->prepare($sql)) {
  ...
  <body>
    <main>
      <section class="container formazas">
        ...
        <hr style="width:50%;height:2px;border-width:0;background-color:red" >
        ...
        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
          ...
          <div class="form-group">
            <input type="submit" name="" value="REGISZTRÁCIO" class="btn btn-block btn-outline-success">
            <input type="reset" name="" value="VISSZAÁLLÍTÁS" class="btn btn-block btn-outline-warning">
          </div>
